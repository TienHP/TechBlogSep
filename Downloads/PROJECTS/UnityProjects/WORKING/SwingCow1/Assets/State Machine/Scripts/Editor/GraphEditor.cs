...
namespace StateMachine{
	public class GraphEditor : EditorWindow {
		...
		private void OnGUI(){
			...
			ToolBarGUI ();
			scroll = GUI.BeginScrollView (new Rect(0,18,position.width,position.height), scroll, new Rect (0, 18, scrollView.x, scrollView.y), true, true);
		...
		protected virtual void ToolBarGUI(){
		...
		protected virtual void DrawGrid(Rect rect,Vector2 scroll)
		{
			...
			this.DrawGridLines(rect,scroll,12f*zoomFactor,kGridMinorColorDark);
			this.DrawGridLines(rect,scroll,120f*zoomFactor, kGridMajorColorDark);
		}
		...
		public void DrawConnection (Vector3 start, Vector3 end,Rect rect, Color color, bool mult)
		{
			if (Event.current.type != EventType.repaint) {
				return;
			}
			...
			Handles.color = color;
			Handles.DrawAAPolyLine (tex, 5f*zoomFactor, new Vector3[] { start, end });
		...
		public void DrawParallelConnection(Vector3 start, Vector3 end, Color color, float size)
		{
			if (Event.current.type != EventType.repaint) {
				return;
			}
			...
			Texture2D tex = (Texture2D)UnityEditor.Graphs.Styles.connectionTexture.image;
			Handles.color = color;
			Handles.DrawAAPolyLine (tex, size, new Vector3[] { start, end });
			
			Vector3 vector3 = end - start;
			Vector3 dir = vector3.normalized;
			Vector3 vector32 = (vector3 * 0.5f) + start;
			vector32 = vector32 - (cross * 0.5f);
			Vector3 center = vector32 + dir;
			
			Handles.DrawAAPolyLine (tex, size, new Vector3[] { (center) + (cross * 6)+dir*2, (center) - (cross * 6)+dir*2});
			Handles.DrawAAPolyLine (tex, size, new Vector3[] { (center) + (cross * 6)-dir*2, (center) - (cross * 6)-dir*2});
			
		}
		
		public void DrawLine(Vector3 start, Vector3 end, Color color, float size)
		{
			if (Event.current.type != EventType.repaint) {
				return;
			}
